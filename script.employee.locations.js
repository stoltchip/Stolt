...
async function init(){
  ...
  await renderProducts(); // <-- poprawione, teraz poczeka na załadowanie
}

async function loadLocations(){
  const { data, error } = await supabase.from('locations').select('*').order('name');
  if(error){
    info.textContent = 'Fout bij laden locaties: ' + error.message;
    return;
  }
  locations = data || [];
  locSelect.innerHTML = locations.map(l => `<option value="${l.id}">${l.name}</option>`).join('');
  
  // <-- nowa linijka: automatycznie ustaw pierwszą lokalizację
  if (!locSelect.value && locations.length) {
...
async function loadProductsAndVariants(){
  ...
  if(e1){
    info.textContent = 'Fout bij laden producten: ' + e1.message;
    return;
  }
  products = prod || [];
  ...
  if(e2){
    info.textContent = 'Fout bij laden varianten: ' + e2.message;
    return;
  }
  variants = vars || [];
}
...
async function loadStockForLocation(location_id){
  ...
  if(error){
    info.textContent = 'Fout bij laden voorraad: ' + error.message;
    return;
  }
  stockByLoc.clear();
  (data || []).forEach(r => {
...
async function renderProducts(){
  ...
  if(!location_id){
    info.textContent = 'Kies een locatie.';
    return;
  }
  ...
  if (!products.length) {
    info.textContent = 'Geen producten beschikbaar.';
    return;
  }
...
async function onBestelClick(e){
  ...
  if(decErr){
    alert('Fout bij bestellen: ' + decErr.message);
    return;
  }
  if(!decData){
    alert('Niet op voorraad voor deze maat op deze locatie.');
    return;
  }
  ...
  if(insErr){
    alert('Bestelling gelukt, maar opslaan order faalde: ' + insErr.message);
  } else {
